...
static void gsm_rx(struct gsm_modem *gsm)
{
	...
	while (true) {
		/* Since the rx_sem pointer could be switched out asynchronously
		 * by modem_iface_uart_init(), we can't wait for it forever.
		 * Instead, periodically release and retake it.
		 */
		if (k_sem_take(&gsm->gsm_data.rx_sem,
			       K_SECONDS(1)) == -EAGAIN) {
			continue;
		}
...
{
	/* Signal -EAGAIN if AT+CGATT? doesn't give "+CGATT: 1" */
	int error = -EAGAIN;

	/* Expected response is "+CGATT: 0|1" so simply scan for '1' */
	while (len--) {
		if (net_buf_pull_u8(data->rx_buf) == '1') {
			error = 0;
...
static struct modem_cmd check_attached_cmd =
	MODEM_CMD("+CGATT", on_cmd_atcmdinfo_attached, 0U, "");
