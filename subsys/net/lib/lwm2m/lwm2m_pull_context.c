...
static void firmware_transfer(struct k_work *work)
{
	...
	context = CONTAINER_OF(work, struct firmware_pull_context, firmware_work);
	...
	context->firmware_ctx.handle_separate_response = true;
                                      ...
                                      k_timeout_t timeout)
{
	...
	error_code = k_sem_take(&lwm2m_pull_sem, timeout);
	if (error_code) {
		return error_code;
	}
