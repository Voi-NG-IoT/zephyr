				...
				bool last_block, size_t total_size)
{
	...
	error_code =  handle_event(instance, EVENT_PKG_URI_WRITE);
	...
	return lwm2m_pull_context_start_transfer(&instance->pull_context);
...
static struct lwm2m_engine_obj_inst *swmgmt_create(uint16_t obj_inst_id)
{
	...
	INIT_OBJ_RES(SWMGMT_PACKAGE_VERSION_ID, res[index], i, res_inst[index], j, 1, false, true,
		     &instance->package_version, PACKAGE_VERSION_LEN,
			 state_read_pkg_version, NULL, NULL, NULL, NULL);

	INIT_OBJ_RES_OPT(SWMGMT_PACKAGE_ID, res[index], i, res_inst[index], j, 1, false, true,
	                 NULL, NULL, NULL, package_write_cb, NULL);

#ifdef CONFIG_LWM2M_FIRMWARE_UPDATE_PULL_SUPPORT
	INIT_OBJ_RES(SWMGMT_PACKAGE_URI_ID, res[index], i, res_inst[index], j, 1, false, true,
	             instance->package_uri, PACKAGE_URI_LEN,
	             NULL, NULL, NULL, package_uri_write_cb, NULL);
#else
	INIT_OBJ_RES_OPT(SWMGMT_PACKAGE_URI_ID, res[index], i, res_inst[index], j, 1, false, true,
	                 NULL, NULL, NULL, package_uri_write_cb, NULL);
#endif
